    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage your repair sites and central warehouse</p>
        </div>
        <button
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5 mb-6">
          ...
          <label className="flex items-center gap-2 mt-4">
            <input
              ...
              className="rounded border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
            />
            <span className="text-sm text-slate-700 dark:text-slate-300">This is the central warehouse / main location</span>
          </label>
          <div className="flex gap-2 mt-4">
            ...
            <button type="button" onClick={() => { setShowForm(false); setEditing(null); }}
              className="bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-200">
              Cancel
            </button>
          </div>
        </form>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {locations.map((loc) => (
          <div key={loc.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5">
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-semibold text-slate-900 dark:text-slate-100">{loc.name}</h3>
                  {loc.is_central ? (
                    <span className="bg-amber-100 dark:bg-amber-900/40 text-amber-800 text-xs px-2 py-0.5 rounded-full font-medium">Central</span>
                  ) : null}
                </div>
                {loc.address && <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{loc.address}</p>}
              </div>
              <div className="flex gap-1">
                <button onClick={() => startEdit(loc)} className="text-slate-400 dark:text-slate-500 hover:text-blue-600 p-1" title="Edit">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                </button>
                <button onClick={() => handleDelete(loc.id)} className="text-slate-400 dark:text-slate-500 hover:text-red-600 p-1" title="Delete">
