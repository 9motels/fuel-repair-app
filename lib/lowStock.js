...
export function renderDigestHtml(groups, due) {
  const totalItems = groups.reduce((s, g) => s + g.items.length, 0);
  const headBits = [];
...
export function renderDigestText(groups, due) {
  const lines = [];
  const totalItems = groups.reduce((s, g) => s + g.items.length, 0);
  if (totalItems) {
...
export async function runLowStockCheck(db, { trigger = 'cron', resend, fromAddress, toAddress }) {
  ...
  try {
    ...
    if (count === 0 && dueCount === 0) {
      ...
      const html = renderDigestHtml(groups, due);
      const text = renderDigestText(groups, due);
      try {
